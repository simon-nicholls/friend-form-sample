(defproject friend-form-sample "0.0.1-SNAPSHOT"
  ...
  :aliases {"run-dev" ["trampoline" "run" "-m" "friend-form-sample.server/run-dev"]}
  :repl-options  {:init-ns user
                  :init (try
                          (use 'io.pedestal.service-tools.dev)
                          (require 'friend-form-sample.service)
                          ;; Nasty trick to get around being unable to reference non-clojure.core symbols in :init
                          (eval '(init friend-form-sample.service/service #'friend-form-sample.service/routes))
                          (catch Throwable t
  ...
  :main ^{:skip-aot true} friend-form-sample.server)
